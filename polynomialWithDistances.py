...
#
                ...
                if fitted:
                    polyVariablesTemp = poly.transform(
                        [extractedDists])
                    scaledVarTemp = scaler.transform(polyVariablesTemp)
                    xCursorTemp = int(xEstimator.predict(scaledVarTemp)[0])
                    yCursorTemp = int(yEstimator.predict(scaledVarTemp)[0])
                    if xCursorTemp < 0:
                        xCursorTemp = 0
                    if xCursorTemp > 1915:
                        xCursorTemp = 1915
                    ...
                    if yCursorTemp > 1075:
                        yCursorTemp = 1075
                    # a = 0.99
                    # xPredict = int((a * xPredict) + ((1-a)*xCursorTemp))
                    # yPredict = int((a * yPredict) + ((1-a)*yCursorTemp))
                ...
                if calibrating:
                    if calibrateCounterJ < CalibrateDIV[0]:
                        ...
                        scaledVariables = scaler.fit_transform(polyVariables)
                        xEstimator.fit(scaledVariables, xList)
                        yEstimator.fit(scaledVariables, yList)
                        fitted = 1
